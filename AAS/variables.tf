...
variable "instance_type_as" {
  ...
  validation {
    condition     = contains(var.instance_type_list, var.instance_type_as)
    error_message = "Invalid instance type. Please select one of the following: ${join(", ", var.instance_type_list)}."
  }
}

variable "custom_aas_swap" {
  type        = number
  description = "Custom swap size for SAP SWAP AAS server instances"
  default     = 32
}
...
#   type        = bool
#   description = "It prevents the instance from being terminated through the AWS API or the AWS Management Console."
#   default     = true
# }
